...
export default function ExecutionEnvironmentView() {
  const [telemetry, setTelemetry] = useState<SystemTelemetry[]>([]);
  const [networkCalls, setNetworkCalls] = useState<NetworkCall[]>([]);
  const [containerIntegrity, setContainerIntegrity] = useState<
    ContainerIntegrity[]
  >([
    {
      id: "1",
  ...
  useEffect(() => {
    ...
    const generateTelemetry = (): SystemTelemetry => {
      ...
      return {
        ...
        status,
        modelId: ["vts-llm-1", "vts-llm-2", "client-model-a"][
    ...
    const generateNetworkCall = (): NetworkCall => {
      const statuses: NetworkCall["status"][] = [
        "allowed",
        "blocked",
        "suspicious",
      ];
      ...
      return {
        ...
        timestamp: new Date(),
        source: "model-agent",
        destination: ["api.openai.com", "internal-db", "external-api.com"][
          Math.floor(Math.random() * 3)
        ],
        protocol: "HTTPS",
        status,
    ...
    const generateProcessLineage = (): ProcessLineage => ({
      ...
      orchestrator: ["api-gateway", "load-balancer", "direct"][
        Math.floor(Math.random() * 3)
      ],
      apiEndpoint: ["/api/chat", "/api/completion", "/api/embedding"][
        Math.floor(Math.random() * 3)
      ],
      timestamp: new Date(),
    ...
    const interval = setInterval(() => {
      ...
      setProcessLineage((prev) =>
        [generateProcessLineage(), ...prev].slice(0, 20)
      );
    }, 4000);
  ...
  const filteredTelemetry = telemetry.filter(
    (t) => selectedModel === "all" || t.modelId === selectedModel
  );

  const criticalTelemetry = telemetry.filter(
    (t) => t.status === "critical"
  ).length;
  const blockedCalls = networkCalls.filter(
    (n) => n.status === "blocked"
  ).length;
  const compromisedContainers = containerIntegrity.filter(
  ...
  const handleBlockNetwork = (callId: string) => {
    ...
    setNetworkCalls((prev) =>
      prev.map((n) => (n.id === callId ? { ...n, status: "blocked" } : n))
    );
  };
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Monitor system telemetry, network activity, and container
              integrity to detect model hijacks
            </p>
          </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {containerIntegrity.length - compromisedContainers}
                </div>
                <p className="text-xs text-muted-foreground">
                  Verified containers
                </p>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    CPU, memory, and I/O usage per inference call
                  </CardDescription>
                </div>
                <Select value={selectedModel} onValueChange={setSelectedModel}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="client-model-a">
                      Client Model A
                    </SelectItem>
                  </SelectContent>
                </Select>
          ...
          <div className="grid gap-6 md:grid-cols-2">
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        {call.status !== "blocked" && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleBlockNetwork(call.id)}
                          >
                            Block
                          </Button>
                        )}
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Hash verification of model files and embeddings
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                    ...
                    <div key={container.id} className="p-3 border rounded-lg">
                      ...
                      <div className="text-xs text-muted-foreground space-y-1">
                        ...
                        <div className="font-mono">
                          Hash: {container.fileHash}
                        </div>
                        <div>
                          Last check: {container.lastCheck.toLocaleTimeString()}
                        </div>
                        <Badge
                          variant={getStatusColor(container.status)}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Correlation between user sessions, orchestrators, and API
                endpoints
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="text-right text-xs">
                        ...
                        <div className="font-medium">
                          {(process.correlation * 100).toFixed(1)}%
                        </div>
                      </div>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => handleInvestigateProcess(process.id)}
                      >
                        Investigate
                      </Button>
                    </div>
