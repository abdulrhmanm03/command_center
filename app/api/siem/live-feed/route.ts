...
function generateSIEMEvent(): SIEMEvent {
  ...
  return {
    id: `siem-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
...
export async function GET() {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // Send initial burst of 10 events
      for (let i = 0; i < 10; i++) {
        sendEvent()
      }

      // Then send 1 event every 3-4 seconds (15-20 events/min)
