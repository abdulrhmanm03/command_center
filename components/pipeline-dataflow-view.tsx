...
export function PipelineDataflowView() {
  ...
  useEffect(() => {
    ...
    setETLJobs([
      ...
      },
      {
        ...
        transformations: ["Extract embeddings", "Calculate statistics", "Apply filters"],
        timestamp: "1 min ago",
        status: "running",
        duration: "1m 12s",
      ...
      },
      {
        ...
        errorMessage: "Schema mismatch: expected field 'user_id' not found in 234 records",
        logs: [
    ...
    const interval = setInterval(() => {
      setFeatureAccess((prev) =>
        prev.map((fa) => ({
          ...
          avgLatency: Math.max(20, fa.avgLatency + Math.floor(Math.random() * 10) - 5),
        })),
      )
      ...
      setDataSources((prev) =>
        prev.map((ds) => ({
          ...
          latency: Math.max(10, ds.latency + Math.floor(Math.random() * 20) - 10),
        })),
      )
      ...
      setETLJobs((prev) =>
        prev.map((job) => {
          if (job.status === "running") {
            const newProgress = job.recordsProcessed + Math.floor(Math.random() * 500)
            if (newProgress >= 15000) {
              return { ...job, status: "success" as const, recordsProcessed: 15000 }
            }
  ...
  const handleStopETL = (etlId: string) => {
    ...
    setETLJobs((prev) =>
      prev.map((etl) =>
        etl.id === etlId
          ? { ...etl, status: "failed" as const, logs: [...etl.logs, "[STOPPED] Pipeline manually terminated"] }
          : etl,
      ),
    )
  }
  ...
  const handleApproveChange = (driftId: string) => {
    ...
    setModelDrift((prev) => prev.map((drift) => (drift.id === driftId ? { ...drift, authorized: true } : drift)))
  ...
  const filteredFeatureAccess = featureAccess.filter((fa) => {
    ...
    const matchesSearch = searchQuery === "" || fa.feature.toLowerCase().includes(searchQuery.toLowerCase())
    return matchesModel && matchesSearch
  })

  const filteredDataSources = dataSources.filter((ds) => {
    const matchesSource = selectedSource === "all" || ds.type === selectedSource
    const matchesSearch = searchQuery === "" || ds.name.toLowerCase().includes(searchQuery.toLowerCase())
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Pipeline & Dataflow Monitoring</h2>
          <p className="text-muted-foreground mt-2">
            Track data lineage, ETL provenance, and model selection to detect data poisoning and unauthorized changes
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={handleCompare} variant="outline" disabled={selectedItems.size < 2}>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card className="cursor-pointer hover:bg-accent/50 transition-colors">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">ETL transformation failures</p>
          </CardContent>
        </Card>
        ...
        <Card className="cursor-pointer hover:bg-accent/50 transition-colors">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Unusual access patterns</p>
          </CardContent>
        </Card>
        ...
        <TabsContent value="overview" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Visual representation of data movement through the system</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between p-6 bg-muted/30 rounded-lg">
                <div className="flex flex-col items-center gap-2">
                  ...
                  <Badge variant="secondary">{dataSources.length} active</Badge>
                  <div className="text-xs text-muted-foreground">
                    {dataSources.filter((ds) => ds.status === "active").length} healthy
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <div className="text-xs text-muted-foreground">{runningJobs} running</div>
                </div>
                <ArrowRight className="h-6 w-6 text-muted-foreground" />
                <div className="flex flex-col items-center gap-2">
                  <Database className="h-10 w-10 text-green-500" />
                  <span className="text-sm font-medium">Feature Store</span>
                  <Badge variant="secondary">{featureAccess.length} features</Badge>
                  <div className="text-xs text-muted-foreground">{anomalousFeatures} anomalies</div>
                </div>
                <ArrowRight className="h-6 w-6 text-muted-foreground" />
                <div className="flex flex-col items-center gap-2">
                  <Zap className="h-10 w-10 text-orange-500" />
                  <span className="text-sm font-medium">AI Models</span>
                  <Badge variant="secondary">{modelDrift.length} tracked</Badge>
                  <div className="text-xs text-muted-foreground">{unauthorizedDrifts} unauthorized</div>
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <div className="space-y-3">
                  {auditLogs.slice(0, 5).map((log) => (
                    <div key={log.id} className="flex items-start gap-3 text-sm">
                      <div
                        className={`mt-0.5 h-2 w-2 rounded-full ${log.status === "success" ? "bg-green-500" : "bg-red-500"}`}
                      />
                ...
                <div>
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className="font-medium">
                      {Math.round(
                        (dataSources.filter((ds) => ds.status === "active").length / dataSources.length) * 100,
                      )}
                      %
                    </span>
                  </div>
                  <Progress
                    value={(dataSources.filter((ds) => ds.status === "active").length / dataSources.length) * 100}
                  />
                </div>
                <div>
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <span className="font-medium">
                      {Math.round((etlJobs.filter((j) => j.status === "success").length / etlJobs.length) * 100)}%
                    </span>
                  </div>
                  <Progress value={(etlJobs.filter((j) => j.status === "success").length / etlJobs.length) * 100} />
                </div>
                <div>
                  <div className="flex items-center justify-between text-sm mb-2">
                    <span>Feature Quality Score</span>
                    <span className="font-medium">
                      {Math.round(featureAccess.reduce((acc, fa) => acc + fa.qualityScore, 0) / featureAccess.length)}%
                    </span>
                  </div>
                  <Progress
                    value={featureAccess.reduce((acc, fa) => acc + fa.qualityScore, 0) / featureAccess.length}
                  />
                </div>
              </CardContent>
            </Card>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Track where training and inference data originates</CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <div key={source.id} className="border rounded-lg">
                    <div className="flex items-center justify-between p-4">
                      <div className="flex items-center gap-4 flex-1">
                        ...
                        />
                        <Database
                          className={`h-8 w-8 ${source.type === "internal" ? "text-blue-500" : "text-orange-500"}`}
                        />
                        <div className="flex-1">
                          <div className="font-medium">{source.name}</div>
                          <div className="text-sm text-muted-foreground">
                            {source.totalRequests.toLocaleString()} requests • {source.dataVolume} • {source.location}
                          </div>
                          <div className="flex items-center gap-3 mt-1">
                            <div className="text-xs text-muted-foreground">Latency: {source.latency}ms</div>
                            <div className="text-xs text-muted-foreground">Error Rate: {source.errorRate}%</div>
                            <div className="text-xs text-muted-foreground">Last: {source.lastAccess}</div>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={source.type === "internal" ? "default" : "secondary"}>{source.type}</Badge>
                        <Badge
                          variant={
                        ...
                        <Button size="sm" variant="ghost" onClick={() => toggleExpanded(source.id)}>
                        ...
                        <Button size="sm" variant="outline" onClick={() => handleViewDetails(source, "source")}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Transformations applied to data before model ingestion</CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                  ...
                  {selectedItems.size > 0 && (
                    <Button variant="outline" size="sm" onClick={() => handleBatchAction("retry")}>
                  ...
                  <div key={job.id} className="border rounded-lg">
                    <div className="p-4">
                        ...
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-muted-foreground mt-1">
                              {job.timestamp} • Duration: {job.duration} • {job.recordsProcessed.toLocaleString()}{" "}
                              records
                            </div>
                            <div className="text-xs text-muted-foreground mt-1">
                              Owner: {job.owner} • Schedule: {job.schedule} • Retries: {job.retryCount}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          {job.status === "running" && (
                            <Button size="sm" variant="outline" onClick={() => handleStopETL(job.id)}>
                              <Ban className="h-4 w-4" />
                            </Button>
                          )}
                          {job.status === "failed" && (
                            <Button size="sm" variant="outline" onClick={() => handleRetryETL(job.id)}>
                              <Play className="h-4 w-4" />
                            </Button>
                          )}
                          <Button size="sm" variant="ghost" onClick={() => toggleExpanded(job.id)}>
                      ...
                      <div className="flex items-center gap-2 text-sm mb-3">
                        ...
                        <span className="text-muted-foreground">{job.target}</span>
                      </div>
                      ...
                      {job.status === "running" && (
                        <div className="mb-3">
                          <div className="flex items-center justify-between text-sm mb-1">
                            ...
                            <span className="font-medium">{Math.round((job.recordsProcessed / 15000) * 100)}%</span>
                          </div>
                          <Progress value={(job.recordsProcessed / 15000) * 100} />
                        </div>
                      )}
                    ...
                    {expandedItems.has(job.id) && (
                      <div className="border-t p-4 bg-muted/30 space-y-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button size="sm" variant="outline" onClick={() => handleViewDetails(job, "etl")}>
                          ...
                          <Button size="sm" variant="destructive" onClick={() => handleDeleteETL(job.id)}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Frequency and type of features requested by models</CardDescription>
                </div>
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
              ...
              <div className="space-y-3">
                {filteredFeatureAccess.map((fa) => (
                  <div key={fa.id} className="border rounded-lg">
                    <div className="flex items-center justify-between p-4">
                      <div className="flex items-center gap-4 flex-1">
                        <Checkbox checked={selectedItems.has(fa.id)} onCheckedChange={() => toggleSelected(fa.id)} />
                        <div className="flex-1">
                          <div className="font-medium flex items-center gap-2">
                            {fa.feature}
                            {fa.anomaly && <AlertTriangle className="h-4 w-4 text-yellow-500" />}
                          </div>
                          <div className="text-sm text-muted-foreground">
                            {fa.model} • {fa.lastAccess}
                          </div>
                          <div className="flex items-center gap-3 mt-1">
                            <div className="text-xs text-muted-foreground">Latency: {fa.avgLatency}ms</div>
                            <div className="text-xs text-muted-foreground">Size: {fa.dataSize}</div>
                            <div className="text-xs text-muted-foreground">Quality: {fa.qualityScore}%</div>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          <div className="font-bold text-lg">{fa.frequency}</div>
                          <div className="text-xs text-muted-foreground">requests</div>
                        </div>
                        {fa.anomaly && (
                          <Button size="sm" variant="outline" onClick={() => handleInvestigateAnomaly(fa.id)}>
                            <Search className="h-4 w-4" />
                          </Button>
                        )}
                        <Button size="sm" variant="ghost" onClick={() => toggleExpanded(fa.id)}>
                    ...
                    {expandedItems.has(fa.id) && (
                      <div className="border-t p-4 bg-muted/30 space-y-3">
                        <div>
                          <div className="text-sm font-medium mb-2">Usage History (Last 3 intervals)</div>
                            ...
                            {fa.usageHistory.map((hist, idx) => (
                              <div key={idx} className="flex-1 flex flex-col items-center gap-1">
                                <div
                                  ...
                                  style={{
                                    height: `${(hist.count / Math.max(...fa.usageHistory.map((h) => h.count))) * 100}%`,
                                  }}
                                />
                                <div className="text-xs text-muted-foreground">{hist.time}</div>
                                <div className="text-xs font-medium">{hist.count}</div>
                              </div>
                            ))}
                          </div>
                        </div>
                        {fa.anomaly && (
                          <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 rounded">
                            ...
                            <div className="text-sm">
                              Unusual spike in request frequency. Current rate is{" "}
                              {(fa.usageHistory[2].count / fa.usageHistory[0].count).toFixed(1)}x higher than baseline.
                              Recommend investigating for potential data exfiltration or abuse.
                            </div>
                          </div>
                        )}
                        <div className="flex items-center gap-2">
                          <Button size="sm" variant="outline" onClick={() => handleViewDetails(fa, "feature")}>
                            <Eye className="h-4 w-4 mr-1" />
                            Full Analytics
                          </Button>
                          <Button size="sm" variant="outline">
        ...
        <TabsContent value="drift" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Unexpected model or parameter changes with impact analysis</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {modelDrift.map((drift) => (
                  <div
                    key={drift.id}
                    className={`border rounded-lg ${!drift.authorized ? "border-destructive bg-destructive/5" : ""}`}
                  >
                    <div className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="font-medium flex items-center gap-2">
                            {drift.model}
                            {!drift.authorized && <AlertTriangle className="h-4 w-4 text-destructive" />}
                          </div>
                          <div className="text-sm text-muted-foreground mt-1">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant="outline">{drift.changeType}</Badge>
                          {drift.authorized ? (
                      ...
                      <div className="space-y-2 mb-3">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <code className="text-xs bg-muted px-2 py-1 rounded">{drift.previousVersion}</code>
                        </div>
                        <div className="flex items-center gap-2 text-sm">
                          <span className="text-muted-foreground">To:</span>
                          <code className="text-xs bg-muted px-2 py-1 rounded">{drift.currentVersion}</code>
                        </div>
                      </div>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Progress value={drift.impactScore * 10} className="flex-1" />
                          <span className="text-sm font-medium">{drift.impactScore}/10</span>
                        </div>
                      </div>

                      <div className="mb-3">
                        <div className="text-sm text-muted-foreground mb-2">Affected Pipelines</div>
                        <div className="flex flex-wrap gap-2">
                      ...
                      {drift.reason && (
                        <div className="text-sm text-muted-foreground italic mb-3">Reason: {drift.reason}</div>
                      )}
                      ...
                      {!drift.authorized && (
                        <div className="flex items-center gap-2 pt-3 border-t">
                          ...
                          <Button size="sm" variant="outline" onClick={() => handleApproveChange(drift.id)}>
                            <Check className="h-4 w-4 mr-1" />
                            Approve
                          </Button>
                          <Button size="sm" variant="destructive" onClick={() => handleRejectChange(drift.id)}>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>Complete history of all pipeline and model changes</CardDescription>
                </div>
                <div className="flex items-center gap-2">
            ...
            <CardContent>
              <div className="space-y-2">
                {auditLogs.map((log) => (
                  ...
                  >
                    <div
                      className={`mt-1 h-2 w-2 rounded-full ${log.status === "success" ? "bg-green-500" : "bg-red-500"}`}
                    />
                    <div className="flex-1">
      ...
      <Dialog open={!!selectedDetail} onOpenChange={() => setSelectedDetail(null)}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Comprehensive information and metrics</DialogDescription>
          </DialogHeader>
          {selectedDetail && (
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-lg font-semibold">{selectedDetail.name}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Type</Label>
                      <div>
                        <Badge variant={selectedDetail.type === "internal" ? "default" : "secondary"}>
                          {selectedDetail.type}
                        </Badge>
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="text-lg font-semibold">{selectedDetail.totalRequests.toLocaleString()}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Data Volume</Label>
                      <div className="text-lg font-semibold">{selectedDetail.dataVolume}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Location</Label>
                      <div className="text-lg font-semibold">{selectedDetail.location}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Status</Label>
                      <div>
                        <Badge variant={selectedDetail.status === "active" ? "default" : "outline"}>
                          {selectedDetail.status}
                        </Badge>
                      </div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Average Latency</Label>
                      <div className="text-lg font-semibold">{selectedDetail.latency}ms</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Error Rate</Label>
                      <div className="text-lg font-semibold">{selectedDetail.errorRate}%</div>
                    </div>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Dependencies</Label>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedDetail.dependencies.map((dep: string, idx: number) => (
                        <Badge key={idx} variant="outline">
                          {dep}
                        </Badge>
                      ))}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedDetail.tags.map((tag: string, idx: number) => (
                        <Badge key={idx} variant="secondary">
                          {tag}
                        </Badge>
                      ))}
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-lg font-semibold">{selectedDetail.duration}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Records Processed</Label>
                      <div className="text-lg font-semibold">{selectedDetail.recordsProcessed.toLocaleString()}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Owner</Label>
                      <div className="text-lg font-semibold">{selectedDetail.owner}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Schedule</Label>
                      <div className="text-lg font-semibold">{selectedDetail.schedule}</div>
                    </div>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Data Flow</Label>
                  ...
                  <div>
                    <Label className="text-muted-foreground">Transformations Applied</Label>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedDetail.transformations.map((t: string, i: number) => (
                  ...
                  {selectedDetail.errorMessage && (
                    <div className="p-3 bg-destructive/10 border border-destructive/20 rounded">
                      ...
                      <div className="text-sm mt-1">{selectedDetail.errorMessage}</div>
                    </div>
                  )}
                </>
              )}
                ...
                <>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="text-lg font-semibold">{selectedDetail.feature}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Model</Label>
                      <div className="text-lg font-semibold">{selectedDetail.model}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Request Frequency</Label>
                      <div className="text-lg font-semibold">{selectedDetail.frequency.toLocaleString()}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Average Latency</Label>
                      <div className="text-lg font-semibold">{selectedDetail.avgLatency}ms</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Data Size</Label>
                      <div className="text-lg font-semibold">{selectedDetail.dataSize}</div>
                    </div>
                    <div>
                      <Label className="text-muted-foreground">Quality Score</Label>
                      <div className="text-lg font-semibold">{selectedDetail.qualityScore}%</div>
                    </div>
                  </div>
                  <div>
                    <Label className="text-muted-foreground">Usage Trend</Label>
                    <div className="flex items-end gap-2 h-32 mt-2">
                      {selectedDetail.usageHistory.map((hist: any, idx: number) => (
                        <div key={idx} className="flex-1 flex flex-col items-center gap-1">
                          <div
                            className="w-full bg-blue-500 rounded-t"
                            style={{
                              height: `${(hist.count / Math.max(...selectedDetail.usageHistory.map((h: any) => h.count))) * 100}%`,
                            }}
                          />
                          <div className="text-xs text-muted-foreground">{hist.time}</div>
                          <div className="text-xs font-medium">{hist.count}</div>
                  ...
                  {selectedDetail.anomaly && (
                    <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 rounded">
                      <Label className="text-yellow-700 dark:text-yellow-400">Anomaly Analysis</Label>
                      <div className="text-sm mt-1">
                        Unusual spike in request frequency detected. Current rate is{" "}
                        {(selectedDetail.usageHistory[2].count / selectedDetail.usageHistory[0].count).toFixed(1)}x
                        higher than baseline average. Recommend investigating for potential data exfiltration or abuse.
                      </div>
                    </div>
                  )}
                </>
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Configure a new ETL transformation pipeline</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                toast({ title: "Pipeline Created", description: "New ETL pipeline has been configured successfully." })
        ...
        <DialogContent className="max-w-4xl">
          ...
          <div className="grid grid-cols-2 gap-4">
            {compareItems.slice(0, 2).map((item, idx) => (
              <Card key={idx}>
                <CardHeader>
                  <CardTitle className="text-base">{item?.name || item?.feature}</CardTitle>
                </CardHeader>
